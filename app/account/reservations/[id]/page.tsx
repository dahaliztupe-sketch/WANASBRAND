...
export default function ReservationDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!reservation) return <div className="text-center py-20">{t.orderDetails.notFound}</div>;
        ...
        <aside className="space-y-12">
          ...
          <section className="space-y-8 bg-inverted/[0.02] p-8 border border-primary/5">
            ...
            <div className="space-y-4 text-sm">
              <div className="flex justify-between text-primary/60">
                ...
                <span><span dir="ltr">EGP {reservation.financials.subtotal.toLocaleString()}</span></span>
              </div>
              <div className="flex justify-between text-primary/60">
                <span>{t.orderDetails.vat}</span>
                <span><span dir="ltr">EGP {reservation.financials.vat.toLocaleString()}</span></span>
              </div>
              <div className="flex justify-between text-primary/60">
                <span>{t.orderDetails.shipping}</span>
                <span><span dir="ltr">EGP {reservation.financials.shippingFee.toLocaleString()}</span></span>
              </div>
              ...
              <div className="flex justify-between text-xl font-serif text-primary">
                ...
                <span><span dir="ltr">EGP {reservation.totalAmount.toLocaleString()}</span></span>
              </div>
            </div>
          </section>
          ...
          <section className="space-y-8 bg-inverted/[0.02] p-8 border border-primary/5">
            ...
            <div className="space-y-4 text-sm text-primary/60 leading-relaxed">
              <p className="font-medium text-primary">{reservation.customerInfo.fullName}</p>
              <p>{reservation.customerInfo.phone}</p>
              <p>{reservation.customerInfo.address}</p>
            </div>
          </section>
        </aside>
      </div>
