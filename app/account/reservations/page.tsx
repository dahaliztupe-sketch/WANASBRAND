...
export default function OrdersPage() {
  ...
  const fetchReservations = useCallback(async (uid: string) => {
    try {
      const q = query(
    ...
    <div className="space-y-16">
      ...
      <div className="space-y-12">
        {reservations.map((reservation, idx) => {
          const status = statusMap[reservation.status] || { label: reservation.status, color: 'bg-gray-50 text-gray-600 border-gray-100' };
          return (
            <motion.div
              key={reservation.id}
              ...
              <div className="p-8 md:p-12">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex items-center gap-4 text-xs text-primary/60">
                      <div className="flex items-center gap-2">
                        ...
                        {new Date(reservation.createdAt).toLocaleDateString(locale === 'ar' ? 'ar-EG' : 'en-US', { month: 'long', day: 'numeric', year: 'numeric' })}
                      </div>
                      <div className="w-1 h-1 rounded-full bg-primary/10" />
                      <div className="flex items-center gap-2">
                        <ShoppingBag strokeWidth={1} className="w-3 h-3" />
                        {reservation.items.length} {reservation.items.length === 1 ? t.ordersList.piece : t.ordersList.pieces}
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex flex-col md:flex-row justify-between items-end gap-8">
                  <div className="flex -space-x-4 overflow-hidden">
                    {reservation.items.slice(0, 4).map((item, i) => (
